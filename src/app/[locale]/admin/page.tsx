...
export default async function AdminPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const stats = [
    { label: t("total"), value: total, color: "bg-gray-50 border-gray-200" },
    { label: t("pending"), value: pending, color: "bg-amber-50 border-amber-200" },
    { label: t("exam"), value: examScheduled, color: "bg-blue-50 border-blue-200" },
    { label: t("accepted"), value: accepted, color: "bg-green-50 border-green-200" },
    { label: t("rejected"), value: rejected, color: "bg-red-50 border-red-200" },
  ];

  return (
    <div className="min-h-screen bg-[#f0f7f2]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h1 className="text-3xl font-bold text-[#1b6b3a] mb-8">{t("title")}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Link href={`/${locale}/admin/applications`} className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow flex items-center gap-4">
            ...
            <div>
              <h3 className="font-bold text-[#1b6b3a]">{t("applications")}</h3>
              <p className="text-sm text-gray-500">{t("apps_count", { total })}</p>
            </div>
          </Link>
          <Link href={`/${locale}/admin/teachers`} className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow flex items-center gap-4">
            ...
            <div>
              <h3 className="font-bold text-[#1b6b3a]">{t("teachers")}</h3>
              <p className="text-sm text-gray-500">{t("manage_profiles")}</p>
            </div>
          </Link>
          <Link href={`/${locale}/admin/news`} className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow flex items-center gap-4">
            ...
            <div>
              <h3 className="font-bold text-[#1b6b3a]">{t("news")}</h3>
              <p className="text-sm text-gray-500">{t("manage_posts")}</p>
            </div>
          </Link>
        </div>
      </div>
